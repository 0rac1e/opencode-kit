...
resolve_version() {
    ...
    # No releases or tags — fall back to main branch
...
download_and_extract() {
    ...
    # For branch names (main/master) use the archive endpoint; for versioned releases use releases
    local tarball_url
    if [[ "$version" == "main" || "$version" == "master" ]]; then
        tarball_url="https://github.com/${REPO}/archive/refs/heads/${version}.tar.gz"
    else
        tarball_url="https://github.com/${REPO}/releases/download/${version}/opencode-kit-${version}.tar.gz"
    fi

    if ! fetch "$tarball_url" | tar xz -C "$tmp_dir" 2>/dev/null; then
        # Release asset not found — fall back to main branch archive
        if [[ "$version" != "main" && "$version" != "master" ]]; then
            warn "Release asset not found for ${version}, falling back to main branch..."
            rm -rf "$tmp_dir"
            tmp_dir=$(mktemp -d)
            tarball_url="https://github.com/${REPO}/archive/refs/heads/main.tar.gz"
            if ! fetch "$tarball_url" | tar xz -C "$tmp_dir" 2>/dev/null; then
                rm -rf "$tmp_dir"
                error "Download failed from both release and main branch."
                error "  https://github.com/${REPO}/releases"
                exit 1
            fi
        else
            rm -rf "$tmp_dir"
            error "Download failed. Check that the repository is accessible:"
